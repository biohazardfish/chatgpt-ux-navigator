...
`,
    ...
    it('rejects > 20 agents', async () => {
      const agents: Record<
        string,
        { client_id: string; system: string }
      > = {};
      const order = [];
      for (let i = 1; i <= 21; i++) {
        const id = `Agent${i}`;
...
`,
      ...
      await expect(loadConfig(configPath)).rejects.toThrow(
        /workflow.start.*must be one of/i,
      );
