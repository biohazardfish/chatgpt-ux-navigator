...
export async function createRunLogger(params: CreateRunLoggerParams): Promise<RunLogger> {
	...
	const configPath_ = join(runDir, 'config.yml');
	await writeFile(configPath_, configText, 'utf-8');
	...
	const logger: RunLogger = {
		...
		async writeTurn(msg: AgentMessage, received_turns: number[]): Promise<void> {
			...
			await renameAtomic(messageTmpPath, messagePath);
			...
			await renameAtomic(transcriptTmpPath, transcriptPath);
		},
		...
		async writeJudge(turn: number, decision: JudgeDecision, created_at: string): Promise<void> {
			...
			await renameAtomic(judgeTmpPath, judgePath);
		},
		...
		async finalize(result: {
			...
			await renameAtomic(runJsonTmpPath, runJsonPath);
		},
	};
...
/**
 * Atomic rename: use fs/promises rename which handles overwriting
 * For transcript.md, we allow overwriting (multiple appends)
 * For message files, there should only be one write per file
 */
async function renameAtomic(tmpPath: string, targetPath: string): Promise<void> {
