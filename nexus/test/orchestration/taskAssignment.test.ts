...
describe('prepareExecutableTask', () => {
    ...
    beforeEach(async () => {
        rootDir = join(
            tmpdir(),
            `nexus-task-assignment-${Date.now()}-${Math.random().toString(36).slice(2)}`
        );
    ...
    it('throws TaskNotExecutableError when task not found', async () => {
        ...
        expect(() => prepareExecutableTask(result.project, 'T-999')).toThrow(
...
describe('validateTransition', () => {
    ...
    it('throws TaskNotExecutableError for invalid transitions', () => {
        expect(() => validateTransition('T-001', 'completed', 'running')).toThrow(
            TaskNotExecutableError
        );
...
describe('assignTaskForExecution', () => {
    ...
    beforeEach(async () => {
        rootDir = join(
            tmpdir(),
            `nexus-task-assign-${Date.now()}-${Math.random().toString(36).slice(2)}`
        );
        await mkdir(rootDir, {recursive: true});
    });
