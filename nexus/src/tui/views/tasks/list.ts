...
function getOrCreateView(
    ...
    const list = new SelectRenderable(container.ctx, {
        ...
        options:
            tasks.length > 0 ? formatListOptions(tasks) : [{name: '(no tasks)', description: ''}],
...
export function render(
    container: any,
    state: TuiState,
    config: {setState: TuiSetState; getState: () => TuiState}
): void {
    ...
    if (
        ...
        state.selectedTaskIndex < view.tasks.length
    ) {
